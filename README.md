...
- 🎯 Entry points to execute your program using `python -m <studious_eureka_moment>` or `$ studious_eureka_moment` with basic CLI argument parsing.
...
---
# studious_eureka_moment Flask Application

Awesome studious_eureka_moment created by ash-oakenthorne
...
```bash
git clone https://github.com/ash-oakenthorne/studious_eureka_moment studious_eureka_moment
cd studious_eureka_moment
make install
```
...
```bash
pip install studious_eureka_moment
```
...
```bash
$ studious_eureka_moment
```

or

```bash
$ python -m studious_eureka_moment
```
...
```bash
studious_eureka_moment create-db   # run once
studious_eureka_moment populate-db  # run once (optional)
studious_eureka_moment add-user -u admin -p 1234  # ads a user
studious_eureka_moment run
```
